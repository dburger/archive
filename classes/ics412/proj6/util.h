...
struct ipc_ids {
	...
	spinlock_t ary;
        /* struct ipc_id* entries; removed by dburger */
        struct list_head entries; /* added by dburger, head of list */
};
...
extern inline struct kern_ipc_perm* ipc_get(struct ipc_ids* ids, int id)
{
	...
	out = ids->entries[lid].p; removed by dburger
        */
        /* block added by dburger */
        {
          struct list_head *p;
          list_for_each(p, &ids->entries) {
            out = list_entry(p, struct kern_ipc_perm, list);
            if (lid == out->lid) return out;
          }
        }
	return NULL;
}
