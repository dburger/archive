...
typedef struct {
     LONG digest[ 5 ];     /* Message digest */
      ...
      unsigned long data[ 16 ];      /* SHS data buffer */
      } SHS_INFO;
...
#define subRound1(count) { temp = S( 5, A ) + f1( B, C, D ) + E + W[ count ] + K1; E = D; D = C; C = S( 30, B ); B = A; A = temp; }
#define subRound2(count) { temp = S( 5, A ) + f2( B, C, D ) + E + W[ count ] + K2; E = D; D = C; C = S( 30, B ); B = A; A = temp; }
#define subRound3(count) { temp = S( 5, A ) + f3( B, C, D ) + E + W[ count ] + K3; E = D; D = C; C = S( 30, B ); B = A; A = temp; }
#define subRound4(count) { temp = S( 5, A ) + f4( B, C, D ) + E + W[ count ] + K4; E = D; D = C; C = S( 30, B ); B = A; A = temp; }
...
void main(int argc, char *argv[])
{
    ...
    shsInit( &shsInfo );
    shsUpdate( &shsInfo, ( BYTE * ) str, length);
    shsFinal( &shsInfo );
    printf("File name: %s--length: %ld bytes\n", filename, length);
    for(i = 0; i<5; i++) printf("%08x  ", shsInfo.digest[i]);
	printf("\n");
}
