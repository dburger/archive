...
/* here cluster == -1 will used to indicate cluster contains no valid data */
...
struct file_descriptor {
  ...
  struct dir_entry de;
  int de_offset;
  struct cluster_buffer buffer[BUFFER_CNT];
  int next_buffer;
};
 ...
 *                  locate
 *
 * @return    cluster containing directory entry, or -1 if it is invalid
 */
...
void fd_close() {
  ...
  /* force any buffered writes to occur */
  for (i = 0; i < FILES; i++) {
    if (fd_table[i].active) fd_fclose(i);
  }
...
int fd_fopen(char *name, int mode) {
  char *buf, *fat_copy;
  int de_offset, dir_cluster, entry, entry_cluster, i, j;
  struct dir_entry de;
  ...
  if ((entry_cluster = locate_entry(fat_copy, dir_cluster, name, &de, buf,
     ...
     * file, making sure the file name is valid.
     */
    if (mode == O_READ || !is_dosname(name)) {
      free(buf);
      free(fat_copy);
      return -1;
    }
  ...
  /* now look for an available file handle */
  for (i = 0; i < FILES; i++) {
  ...
  /* file descriptor at i is available, set it up for use */
  fd_table[i].active = 1;
  ...
  /* set up buffers for read ahead / write behind */
  for (j = 0; j < BUFFER_CNT; j++) {
    fd_table[i].buffer[j].buf = (char *)malloc(dg.bytes_per_clust);
    fd_table[i].buffer[j].cluster = -1;
  }

  /*
   * Setting past the index for the last buffer indicates the need
   * to start back at the first buffer on the next iteration of read / write.
   */
  fd_table[i].next_buffer = BUFFER_CNT;
...
int fd_fread(int fhandle, char *buf, int len) {
  ...
  while (data_read < len && fdp->pos < fdp->size) {
    ...
    if (fdp->next_buffer == BUFFER_CNT) {
      /* need to reload the buffers */
      i = 0;
      while (i < BUFFER_CNT && (fdp->next_cluster & 0xff8) != 0xff8 &&
             fdp->next_cluster != 0) {
        if (ll_fd_read(fdp->next_cluster, fdp->buffer[i].buf) < 0) return -1;
        fdp->buffer[i].cluster = fdp->next_cluster;
        fdp->next_cluster = read_fat(fat_cache, fdp->next_cluster);
        i++;
      }
      /* and set the next read to come from the first buffer */
      fdp->next_buffer = 0;
    }
    ...
    memcpy(buf + data_read, fdp->buffer[fdp->next_buffer].buf + start, size);
    data_read += size;
    fdp->pos += size;

    if (start + size == dg.bytes_per_clust) fdp->next_buffer++;
  }
...
int fd_fwrite(int fhandle, char *buf, int len) {
  int data_written = 0, i, size, start;
  struct file_descriptor *fdp;
  ...
  while (data_written < len) {
    ...
    if (fdp->next_buffer == BUFFER_CNT) {
      ...
      /* check for need to write behind */
      i = 0;
      while (i < BUFFER_CNT) {
        if (fdp->buffer[i].cluster != -1) {
          if (ll_fd_write(fdp->buffer[i].cluster, fdp->buffer[i].buf) < 0) {
            return -1;
          }
        }
        i++;
      ...
      /*
       * Mark those not loaded with pre-existing data as invalid so a write
       * behind on close won't attempt to write them.
       */
      while (i < BUFFER_CNT) fdp->buffer[i++].cluster = -1;

      /* set the next write to go to the first buffer */
      fdp->next_buffer = 0;
    }

    if (fdp->buffer[fdp->next_buffer].cluster == -1) {
      if (fdp->cluster == 0) { /* empty file first write */
        if ((fdp->cluster = fat_free_first(fat_cache, 0xff8)) < 0) return -1;
        fdp->buffer[fdp->next_buffer].cluster = fdp->cluster;
        fdp->next_cluster = 0xff8;
      } else if ((fdp->next_cluster & 0xff8) == 0xff8) { /* needs more clust */
        if ((fdp->buffer[fdp->next_buffer].cluster =
             add_cluster(fat_cache, fdp->cluster)) < 0) return -1;
        /* and leave fdp->next_cluster at 0xff8 */
      }
    }
    ...
    memcpy(fdp->buffer[fdp->next_buffer].buf +start, buf + data_written, size);
  ...
  if (lseek(fd, fdp->de_offset, SEEK_SET) != fdp->de_offset) return -1;

  if (write(fd, &fdp->de, sizeof(fdp->de)) != sizeof(fdp->de)) return -1;
...
int fd_fseek(int fhandle, int offset) {
  ...
  if (offset < fdp->pos || offset >= fdp->size) return -1;

  /* check for possible write behind */
  if (fdp->mode == O_WRITE) {
    i = 0;
    while (i < BUFFER_CNT) {
      if (fdp->buffer[i].cluster != -1) {
        ll_fd_write(fdp->buffer[i].cluster, fdp->buffer[i].buf);
      }
      i++;
    }
  }
  ...
  /* buffered clusters no longer valid, force reload on next read/write */
  fdp->next_buffer = BUFFER_CNT;
...
void fd_fclose(int fhandle) {
  int i;
  struct file_descriptor *fdp;
  if (fhandle >= 0 && fhandle < FILES && fd_table[fhandle].active) {
    fdp = &fd_table[fhandle];
    /* check for need to do last write behind */
    if (fdp->mode == O_WRITE) {
      i = 0;
      while (i < BUFFER_CNT) {
        if (fdp->buffer[i].cluster != -1) {
