...
/*
 * Returns the column containing the blank
 *
 * return  the column containing the blank
 */
int blank_col() {
  return current_blank % rows;
}

/*
 * Returns the row containing the blank
 *
...
int expand() {
  ...
  if (!current_heuristic) {
    printf("%c", inverse_move(search_path[current_depth]));
    return 1;
  }
...
int main(int argc, char *argv[]) {
  ...
  while (!feof(fp) && read_puzzle(fp)) {
    ...
    if (!is_valid(goal_grid)) {
      printf("Start puzzle is invalid!\n");
    } else if (!is_valid(current_grid)) {
