...
struct Node {
  ...
  int blank_pos;
  int depth;
  int manhattan; /* holds manhattan distance for this node */
};
...
void build_manhattan(struct Node *goal, int *manhattan) {
  ...
  for (i=0; i<goal->cells; i++) {
    ...
    for (j=0 ; j<goal->cells; j++) {
      ...
      manhattan[goal->grid[i] * goal->cells + j] = !goal->grid[i] ? 0 : offset;
    }
  }
}
...
int manhattan_distance(struct Node *n, int *manhattan) {
  ...
  for (row=0; row<n->rows; row++) {
    for (col=0; col<n->cols; col++) {
      cell = row * n->cols + col;
      distance += manhattan[n->cells * n->grid[cell] + cell];
    }
  }
  return distance;
}
 ...
 * calling this function.  Call with *manhattan == NULL when you only need
 ...
 * @param *manhattan  manhattan distance table, or null if only undoing
 */
void blank_left(struct Node *n, int *manhattan) {
  int row = blank_row(n);
  int col = blank_col(n);
  ...
  /* adjust manhattan, blank_pos, via, not necessary on undo move */
  if (manhattan) {
    n->manhattan += manhattan[n->cells * temp + n->blank_pos] -
      manhattan[n->cells * temp + new_blank_pos];
 ...
 * @param *manhattan  manhattan distance table, or null if only undoing
 */
void blank_right(struct Node *n, int *manhattan) {
  int row = blank_row(n);
  int col = blank_col(n);
  int new_blank_pos = row * n->rows + col + 1;
  int temp = n->grid[new_blank_pos];
 ...
 * @param *manhattan  manhattan distance table, or null if only undoing
 */
void blank_up(struct Node *n, int *manhattan) {
  int row = blank_row(n);
  int col = blank_col(n);
 ...
 * @param *manhattan  manhattan distance table, or null if only undoing
 */
void blank_down(struct Node *n, int *manhattan) {
  int row = blank_row(n);
  int col = blank_col(n);
  int new_blank_pos = (row + 1) * n->rows + col;
  int temp = n->grid[new_blank_pos];
...
int expand(struct Node *n, int thold, int *new_thold, struct Node *goal,
  ...
  if ((cost = n->depth + n->manhattan) > thold) {
...
int main(int argc, char *argv[]) {
  ...
  while (!feof(fp) && read_puzzle(fp, start, goal, &max_depth)) {
    ...
    if (!is_valid(start)) {
      ...
      /* manhattan distance set in goal puzzle, using backward chaining */
      thold = goal->manhattan =  manhattan_distance(goal, manhattan);
